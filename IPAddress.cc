...
const IPAddress UNSPECIFIED_ADDRESS;
const IPAddress LOOPBACK_ADDRESS("127.0.0.1");
const IPAddress LOOPBACK_NETMASK("255.0.0.0");
const IPAddress ALLONES_ADDRESS("255.255.255.255");

const IPAddress ALL_HOSTS_MCAST("224.0.0.1");
const IPAddress ALL_ROUTERS_MCAST("224.0.0.2");
const IPAddress ALL_DVMRP_ROUTERS_MCAST("224.0.0.4");
const IPAddress ALL_OSPF_ROUTERS_MCAST("224.0.0.5");
const IPAddress ALL_OSPF_DESIGNATED_ROUTERS_MCAST("224.0.0.6");
const IPAddress ALL_IGMPV3_ROUTERS_MCAST("224.0.0.22");

const IPAddress LL_MANET_ROUTERS("224.0.0.109");

const IPAddress ALL_RIP_ROUTERS_MCAST("224.0.0.9");
...
std::string IPAddress::str(bool printUnspec    /* = true */) const
{
...
IPAddress::AddressCategory IPAddress::getAddressCategory() const
{
    if (isUnspecified())
...
bool IPAddress::prefixMatches(const IPAddress& other, int length) const
{
    if (length < 1)
        return true;
    if (length > 31)
...
void IPAddress::_checkNetmaskLength(int length)
{
    if (length < 0 || length > 32)
        throw cRuntimeError("IPAddress: wrong netmask length %d (not in 0..32)", length);
}
...
IPAddress IPAddress::makeBroadcastAddress(IPAddress netmask) const
{
